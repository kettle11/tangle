...
// TODO: This was generated by ChatGPT and needs auditing.
function compute_id_from_ip(ip: string): PeerId {
    const octets = ip.split(":")[0].split(".").map(Number);
    let identifier = 0;
    for (let i = 0; i < octets.length; i++) {
        identifier += octets[i] * 256 ** (3 - i);
    }
    return identifier;
}
