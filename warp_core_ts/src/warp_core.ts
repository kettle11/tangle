...
export class WarpCore {
    ...
    async setup(room_name: string, wasm_binary: Uint8Array, wasm_imports: WebAssembly.Imports): Promise<WarpCore> {
        let room_configuration = {
            ...
            on_state_change: (state: RoomState) => {
                ...
                if (state == RoomState.Connected) {
                    ...
                    let lowest_latency_peer = room.get_lowest_latency_peer();
                    if (lowest_latency_peer) {
                        room.message_specific_peer(lowest_latency_peer, JSON.stringify({
            ...
            on_message: async (peer_id: string, message: any) => {
                ...
                try {
                        ...
                        case (MessageType.RequestHeap): {
                            if (this._bytes_remaining_for_heap_load == 0) {
                                ...
                                room.message_specific_peer(peer_id, JSON.stringify({
                                    ...
                                    time_offset: this._warp_core.time_offset,
                    ...
                    this._loading_heap!.set(message, this._loading_heap!.byteLength - this._bytes_remaining_for_heap_load);
                    this._bytes_remaining_for_heap_load -= message.byteLength;

                    if (this._bytes_remaining_for_heap_load == 0) {
                        let decoded_heap = this._warp_core.gzip_decode(this._loading_heap!);
                        this._loading_heap = undefined;

                        this._warp_core.reset_with_wasm_memory(
                            decoded_heap,
        ...
        let room = await Room.setup(room_configuration);

        let _warp_core = await OfflineWarpCore.setup(wasm_binary, wasm_imports);
        let _warp_core_networking = new WarpCore(room, _warp_core);
        return _warp_core_networking;
    }

    async wasm_call(function_name: string, args: [number]) {
        if (this._bytes_remaining_for_heap_load == 0) {
    ...
    private constructor(room: Room, _warp_core: OfflineWarpCore) {
        this._warp_core = _warp_core;
        this.room = room;
    }
}
